...
微同步把你主动选择的微信文章、飞书文档和文章型网页同步到 Obsidian Vault。

## 功能

- 在 Obsidian 中自动轮询微同步任务，并把 Markdown、图片和附件保存到当前 Vault。
- 浏览器扩展通过一次性 6 位绑定码连接到指定 Vault。
...
1. 安装并启用插件。
2. 在设置中选择 Free 自有对象存储，或登录使用 Pro 托管存储。
3. Free 用户按官网的对象存储教程创建私有 Bucket 和最小权限密钥，点击“测试并保存”。
4. 点击“生成绑定码”，在 Chrome 扩展中输入 6 位绑定码。
5. 在浏览器中主动点击“同步到 Obsidian”，保持 Obsidian 打开等待任务完成。
...
插件只处理用户主动选择同步的内容，以及完成任务所需的任务状态和临时对象。不会读取浏览器历史，不会把对象存储长期密钥交给浏览器扩展，也不用于广告或画像。Free 密钥使用 Obsidian SecretStorage 保存，并由服务端以加密字段保存用于任务处理。

网络请求发送到 `api.wetongbu.com`；旧微信回调域名 `wx.wetongbu.com` 不由插件直接调用。插件会校验任务包哈希、文件大小和安全路径，完成后通知服务端清理远端任务对象。
